...
$sql = "UPDATE tblbooking SET Status=:status WHERE id=:reid AND Status=3";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
$query-> bindParam(':reid',$reid, PDO::PARAM_STR);
$query -> execute();
echo "<script>alert('Return Successfully Accepted');</script>";
echo "<script type='text/javascript'> document.location = 'bookig-details.php?bid=".$reid."'; </script>";
}
...
echo htmlentities('Return Requested');
...
<?php if($result->Status==3){ ?>
										<tr>	
										<td style="text-align:center" colspan="4">
				<a href="bookig-details.php?reid=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you want to accept this return request?')" class="btn btn-primary"> Accept Return</a>
</td>
</tr>
<?php } ?>
										<?php $cnt=$cnt+1; }} ?>
